...
class OperDiscord(commands.Cog):
    ...
    def __init__(Self, bot):
        ...
        Self.MyChannel.append(
            Self.ChannelDetail('997191187259732068', 'Three Questions Game'))
        Self.MyChannel.append(
            Self.ChannelDetail('1071493292363087964', 'Study Room 1'))
        Self.MyChannel.append(
            Self.ChannelDetail('1071493292363087965', 'Study Room 2'))

    async def PostGeneralInfo(Self, ctx, cHeader: str, DescHeader: str,
                              oField):
        embed = discord.Embed(title=cHeader,
                              description=DescHeader,
                              color=discord.Color.blue())

        embed.set_thumbnail(
            url=
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTvlJsqbDBYjobCSePQghhuHn6Ph5eDhQql6Q&usqp=CAU"
        )
    ...
    def Get_Game_OK(Self) -> list:
        ...
        oData = OperationDB(
            'SEL', 'GAME', ['IDGAME', 'DESCRIPTION'], None,
            "STATUS = 'OK' and IDCHANNEL = '" + str(cIdChannel) + "'", None)
    ...
    def Get_MemberId(Self, cMemberId: str) -> list:
        ...
        oData = OperationDB(
            'SEL', 'DISCORD_USER', ['IDMEMBER', 'USERDESC'], None,
            "IDMEMBER = '" + cMemberId + "' and IDGAME = " + str(nIdGame),
            None)
    ...
    def ValidateRoleCommand(Self, cCommand: str, cRoleCode: str):
        ...
        oData = OperationDB(
            'SEL', 'BOT_ROLE', ["'Y' VALIDATE"], None,
            "ROLEID = (SELECT ROLEID FROM HELPER_ROLE WHERE HELPERCODE = '" +
            cRoleCode + "') AND BOT_COMMAND = '" + cCommand + "'", None)
    ...
    def ValidateRoleByCommand(Self, cMemberId: str, cCommand: str,
                              nIdGame: str, cAplicationId: str):
        ...
        oData = OperationDB(
            'SEL', 'USER_BOT_ROLE', ["'Y' VALIDATE"], None,
            "IDMEMBER = '" + str(cMemberId) + "' AND BOT_COMMAND = '" +
            cCommand + "' AND APLICATIONID = '" + str(cAplicationId) +
            "' AND STATUS = 'OK'", None)
        ...
        #cAplicationId = '1071451684691263538'
        cValidate = ''
    ...
    def ValidateRoleUser(Self, cIdMember: str, cRoleCode: str) -> bool:
        ...
        oData = OperationDB(
            'SEL', 'USER_BOT_ROLE', ["'Y' VALIDATE"], None,
            "ROLEID = (SELECT ROLEID FROM HELPER_ROLE WHERE HELPERCODE = '" +
            str(cRoleCode.upper()) + "') AND IDGAME = " + str(nIdGame) +
            " AND STATUS = 'OK'", "GROUP BY 1")
    ...
    def ValidateRoleOut(Self, cIdMember: str, cBotCommand: str, nIdGame: str,
                        cAplicationId: str):
        oData = None

        oData = OperationDB(
            'SEL', 'USER_BOT_ROLE', ["'Y' VALIDATE"], None,
            "IDMEMBER = '" + str(cIdMember) + "' AND BOT_COMMAND = '" +
            str(cBotCommand) + "' AND IDGAME = " + str(nIdGame) +
            " AND APLICATIONID = '" + str(cAplicationId) + "'", None)
    ...
    async def GiveRoleUser(Self, ctx, cIdMember: str, cRoleCode: str):
        print('application_id: ' + str('1071451684691263538'))
        #MyRoleCommand = []
        print('give role method')
        oConection = None
        oSelect = []
        oSelect = Self.Get_Game_OK()  #find a active game
        oConection = oSelect[0]
        ...
        bValidate = foo.Validate_User(
            cIdMember, nIdGame)  #Validate if the user is into the game

        if bValidate == False:
            await ctx.send("User **" + str(bot.get_user(int(cIdMember))) +
                           "** is not in the game, check out!")
            return

        bValidate = foo.ValidateRoleUser(
            cIdMember, cRoleCode)  #Validate if the permissions are available
        ...
        oData = OperationDB(
            'SEL', 'BOT_ROLE', ['ROLEID', 'BOT_COMMAND'], None,
            "ROLEID = (SELECT ROLEID FROM HELPER_ROLE WHERE HELPERCODE = '" +
            cRoleCodeUp + "') AND APLICATIONID = '" +
            str('1071451684691263538') + "'", None)
        ...
        for i in range(len(dblista)):  #data from BOT_ROLE
            print(dblista[i][0],
                  dblista[i][1])  #1st [] -> row / 2nd [] -> column
            nIdRole = dblista[i][0]
            cBotCommand = dblista[i][1]

            cValidate = foo.ValidateRoleOut(cIdMember, cBotCommand, nIdGame,
                                            cAplicationId)

            if cValidate == False:

                #Insert a row of a permit-role for a user selected to use a command
                OperationDB('INS', 'USER_BOT_ROLE', [
                    'ROLEID', 'APLICATIONID', 'BOT_COMMAND', 'IDGAME',
                    'IDMEMBER', 'STATUS'
                ], [
                    str(nIdRole), "'" + str(cAplicationId) + "'",
                    "'" + str(cBotCommand) + "'",
                    str(nIdGame), "'" + cIdMember + "'", "'OK'"
                ], None, None)
                ...
                OperationDB(
                    'UPD', 'USER_BOT_ROLE', "STATUS = 'OK'", None,
                    "IDMEMBER = '" + str(cIdMember) + "' AND BOT_COMMAND = '" +
                    str(cBotCommand) + "' AND IDGAME = " + str(nIdGame) +
                    " AND APLICATIONID = '" + str(cAplicationId) + "'", None)
    ...
    async def PostRole(Self, ctx):
        ...
        oData = OperationDB(
            'SEL',
            "(SELECT R.ROLENAME, D.USERDESC FROM DISCORD_USER D,(Select UB.ROLEID, HR.ROLENAME, UB.IDMEMBER, UB.IDGAME from HELPER_ROLE HR, USER_BOT_ROLE UB Where UB.IDGAME = "
            + str(nIdGame) +
            " AND UB.STATUS = 'OK' AND UB.ROLEID = HR.ROLEID AND APLICATIONID = '"
            + str(cAplicationId) +
            "' GROUP BY UB.ROLEID, HR.ROLENAME, UB.IDMEMBER) R WHERE R.IDMEMBER = D.IDMEMBER AND R.IDGAME = D.IDGAME AND D.STATUS = 'OK')",
            ['ROLENAME', 'USERDESC'], None, None, None)
        ...
        embed = discord.Embed(title='**Helper Roles:**',
                              description="",
                              color=discord.Color.blue())
        ...
        for i in range(len(dblista)):  #data from BOT_ROLE
            ...
            embed.add_field(name=str(cRoleName) + ':',
                            value=f"{str(cUserDesc)}",
                            inline=False)
    ...
    async def PostRoleMissing(Self, ctx):
        ...
        oData = OperationDB(
            'SEL', "HELPER_ROLE HR", ['ROLENAME', 'DESCRIPTION', 'HELPERCODE'],
            None,
            "NOT EXISTS(SELECT 'S' from USER_BOT_ROLE UB Where UB.IDGAME = " +
            str(nIdGame) +
            " AND UB.STATUS = 'OK' AND UB.ROLEID = HR.ROLEID AND UB.APLICATIONID = '"
            + str(cAplicationId) + "') AND POST = 'S'", None)
        ...
        embed = discord.Embed(title='We need Helpers, por favor!',
                              description="",
                              color=discord.Color.blue())
        ...
        for i in range(len(dblista)):  #data from BOT_ROLE
            if nReg == 0:
                embed.add_field(
                    name='',
                    value=
                    ':orange_book: :orange_book: :orange_book: :orange_book: :orange_book: :orange_book: :orange_book: :orange_book: :orange_book: :orange_book: :orange_book: :orange_book: :orange_book: :orange_book:',
                    inline=False)
            ...
            embed.add_field(
                name='',
                value=
                f"* Can anybody please be the **{str(cRoleName)}**(*** {str(cHelperCode)} ***)?\n",
                inline=False)
    ...
    async def RemoveRoleUser(Self, ctx, cRoleCode: str):
        ...
        OperationDB(
            'UPD', 'USER_BOT_ROLE', "STATUS = 'OUT'", None,
            "IDGAME = " + str(nIdGame) +
            " AND ROLEID IN(SELECT HR.ROLEID FROM HELPER_ROLE HR WHERE HR.HELPERCODE = '"
            + cRoleCode + "')", None)

    def Get_Point_Rule(Self, cPointCode: str) -> list:
        nIdGame = []
        #look up for a point rule
        oData = OperationDB('SEL', 'POINT_RULE', ['IDPOINT'], None,
                            "POINTCODE = '" + str(cPointCode) + "'", None)
    ...
    def Validate_User(
            Self, cIdMember: str, nIdGame: int
    ) -> bool:  #Validate if the user is into the Voice Channel
        nIdMember = ''
        oData = OperationDB(
            'SEL', 'DISCORD_USER', ['IDGAME', 'IDMEMBER'], None,
            "STATUS = 'OK' AND IDGAME = " + str(nIdGame) +
            " AND IDMEMBER = '" + cIdMember + "'", None)
    ...
    async def ShowScoreCard(Self, ctx):
        ...
        #look up for a point rule
        oData = OperationDB(
            'SEL', 'SCORECARD', ['IDMEMBER', 'SUM(POINT) POINT'], None,
            "IDMEMBER IN(SELECT IDMEMBER FROM DISCORD_USER WHERE STATUS = 'OK') AND IDGAME IN(SELECT IDGAME FROM GAME WHERE STATUS = 'OK')",
            ' GROUP BY IDMEMBER ORDER BY 2 DESC')

        for OneRow in oData:
            print(OneRow[0], OneRow[1])

            oSelect = []
            oSelect = Self.Get_MemberId(str(
                OneRow[0]))  #find a member from DISCORD_USER's table
            oConection = oSelect[0]
            cUserDesc = oSelect[2]
            oConection.close()

            MyScore.append(
                Self.ScoreCardDetail(OneRow[0], cUserDesc, OneRow[1]))
        ...
        embed = discord.Embed(title='',
                              description="The Three Questions' Game",
                              color=discord.Color.blue())
        ...
        for Row in MyScore:
            ...
            embed.add_field(name=str(Row.UserDesc) + ':',
                            value=f"{str(Row.nPointTotal)}")
    ...
    async def info(Self, ctx):
        ...
        for Row in MyScore:
            while nCont in range(len(MyScore)):
                print(nCont, Row[nCont].IdMember, Row[nCont].UserDesc,
                      Row[nCont].nPointTotal)
        ...
        for Row in MyScore:
            for i in range(len(MyScore)):
                ...
                embed.add_field(name=str(Row[i].UserDesc) + ':',
                                value=f"{str(Row[i].nPointTotal)}")
    ...
    async def stpgame(Self, ctx):
        ...
        OperationDB('UPD', 'GAME', "STATUS = 'END'", None,
                    "IDGAME = " + str(nIdGame), None)

        #kick out all member from the game by the status: OK -> OUT
        OperationDB('UPD', 'DISCORD_USER', "STATUS = 'OUT'", None,
                    "IDGAME = " + str(nIdGame), None)

        #remove all permissions of all users of the current game
        OperationDB('UPD', 'USER_BOT_ROLE', "STATUS = 'OUT'", None,
                    "IDGAME = " + str(nIdGame), None)

        await foo.PostGeneralInfo(
            ctx, 'GAME OVER', "The Three Questions' Game", [
                "Game over, thanks for participating!",
                "Juego finalizado, gracias por participar!"
            ])
    ...
    async def allmembers(Self, ctx):  #, member : discord.member
        ...
        for member in members:
            MyMember.append(
                Self.MemberDetail(member.id, member.name, channel.id))

        for i in range(len(MyMember)):
            print('Channel Id: ' + str(MyMember[i].ChannelRoomId) + ' id: ' +
                  str(MyMember[i].MemberId) + ' User: ' +
                  str(MyMember[i].MemberName))
            '''
            oSelect = []
            ...
            #if len(nMemberId) == 0:
            #Insert a row of a game of a Channel Room
            OperationDB('INS', 'DISCORD_USER', [
                'IDMEMBER', 'IDGAME', 'IDCHANNEL', 'USERDESC', 'DATEREG',
                'HELPER', 'PLAYER', 'STATUS'
            ], [
                "'" + str(MyMember[i].MemberId) + "'",
                str(nIdGame), "'" + str(cIdChannel) + "'",
                "'" + str(MyMember[i].MemberName) + "'",
                "'" + Self.Get_Time() + "'", "'N'", "'Y'", "'OK'"
            ], None, None)

            #oConection.close()

    @commands.Cog.listener()
    async def SavePoint(Self, ctx, cPointCode: str, cMemberId: str,
                        nPoint: int, cSource: str):
        ...
        oData = OperationDB(
            'SEL', 'DISCORD_USER', ['IDGAME', 'IDMEMBER'], None,
            "STATUS = 'OK' AND IDGAME = " + str(nIdGame) +
            " AND IDMEMBER = '" + cMemberIdClean + "'", None)
        ...
        if len(nIdMember) > 0:
            ...
            OperationDB('INS', 'SCORECARD', [
                'IDGAME', 'IDPOINT', 'IDMEMBER', 'DIS_IDGAME', 'POINT',
                'DATEREG'
            ], [
                str(nIdGame),
                str(nIdPoint), "'" + str(nIdMember) + "'",
                str(nIdGame),
                str(nPoint), "'" + foo.Get_Time() + "'"
            ], None, None)
...
async def on_voice_state_update(member: discord.Member, before, after):
    ...
    if str(after.channel) == 'Lounge':
        print('channel: ' + str(before.channel) + ' - ' + str(after.channel) +
              ' Id Channel: ' + str(foo.GetChannelId(str(after.channel))))
    ...
    if before.channel is not None:  #User has left a voice channel
        ...
        oSelect = []
        oSelect = foo.Get_MemberId(str(
            member.id))  #find a member from DISCORD_USER's table
        oConection = oSelect[0]
        nIdMember = oSelect[1]
        ...
        if len(nIdMember) > 0:
            ...
            #update the state's user in the table DISCORD_USER to OUT
            OperationDB(
                'UPD', 'DISCORD_USER', "STATUS = 'OUT'", None, "IDMEMBER = '" +
                str(member.id) + "' and IDGAME = " + str(nIdGame), None)
        ...
        if len(memids) > 0:
            ...
            if len(memids) > 0:
                ...
                if len(nIdMember) > 0:
                    ...
                    OperationDB(
                        'UPD', 'DISCORD_USER', "STATUS = 'OK'", None,
                        "IDMEMBER = '" + str(member.id) + "' and IDGAME = " +
                        str(nIdGame), None)
                    print('Member in DISCORD_USER: ' + nIdMember)

                elif len(nIdMember) <= 0:

                    cIdChannel = str(foo.GetChannelId(str(
                        after.channel)))  #find the ChannelId

                    #Insert a member into the game
                    OperationDB('INS', 'DISCORD_USER', [
                        'IDMEMBER', 'IDGAME', 'IDCHANNEL', 'USERDESC',
                        'DATEREG', 'HELPER', 'PLAYER', 'STATUS'
                    ], [
                        "'" + str(member.id) + "'",
                        str(nIdGame), "'" + str(cIdChannel) + "'",
                        "'" + str(member.name) + "'",
...
async def startgame(ctx):
    ...
    if not str(cAuthor) in cOwner:
        ...
        cApplication = '1071451684691263538'

        bValidate = foo.ValidateRoleByCommand(ctx.author.id, cFunctionName,
                                              nIdGame, cApplication)

        if bValidate == False:  #Validate if the role was used
            await ctx.send("THE USER ( " + str(ctx.author) +
                           " ) **DOESN'T HAVE PERMISSION** TO USING: **" +
                           str(cFunctionName) + "**")
            return
        ###########################################
    ...
    if bStatus:  #find a active game
        await foo.PostGeneralInfo(ctx, 'WARNING', 'Exists a game started', [
            "To start a new game, first you need close the current game with: !stopgame"
        ])
        return
...
async def stopgame(ctx):
    ...
    cAplication = '1071451684691263538'
    ...
    if bStatus == False:  #find a active game
        await foo.PostGeneralInfo(
            ctx, 'WARNING', "Doesn't exists a game started", [
                "To close a current game, first you need start a new game with: !startgame"
            ])
        return
...
async def savescore(ctx, cPointCode: str, cMemberId: str, nPoint: int):
    ...
    if bStatus == False:  #find a active game
        await foo.PostGeneralInfo(
            ctx, 'WARNING', "Doesn't exists a game started",
            ["You need start a new game with: !startgame"])
        return
...
async def stop(ctx):  #Sop Timer
    ...
    if 30 <= LastTime <= 75 and cProcedureGlobal.upper() == 'P':
        ...
        await foo.SavePoint(ctx, 'R', cMemberGlobal, 3,
                            'stop')  #Registry a Bonus Point
        ...
        embed.add_field(
            name='BONUS Point by Presentation' + ':',
            value="+3 for Presentations that finish between 30 to 75 seconds")
    ...
    if 15 <= LastTime <= 40 and cProcedureGlobal.upper() == 'Q':
        ...
        await foo.SavePoint(ctx, 'R', cMemberGlobal, 1,
                            'stop')  #Registry a Bonus Point
        ...
        embed.add_field(
            name='BONUS Point by Follow-Question' + ':',
            value="+1 For each Follow-Up Response between 15 - 40 seconds")
...
async def giverole(ctx, cIdMember: str, cRoleCode: str):
    ...
    bStatus = foo.Get_GameStatus(
    )  #if not exists a OK's game, then show a warning's message and cannot continues
...
async def postcopy(ctx, cProcedure: str):
    ...
    cApplication = '1071451684691263538'
    bValidate = foo.ValidateRoleByCommand(ctx.author.id, cFunctionName, nIdGame, cApplication)
